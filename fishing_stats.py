...
def main():
    ...
    #filtered_data.loc[:,'DateTime']=pd.to_datetime(filtered_data['DateTime'], format="%m/%d/%Y %I:%M:%S %p",errors='coerce').dt.tz_localize("US/Eastern")
    filtered_data.loc[:,'DateTime']=pd.to_datetime(filtered_data['DateTime'], format="%m/%d/%Y %H:%M:%S",errors='coerce').dt.tz_localize("US/Eastern")
